...
def get_portfolio_payment_behavior_metrics(paid_invoices: pd.DataFrame) -> dict:
...
def render_portfolio_payment_behavior_strip(paid_invoices: pd.DataFrame):
    metrics = get_portfolio_payment_behavior_metrics(paid_invoices)

    if not metrics.get("has_data"):
        with ui.card().classes("w-full p-4 mb-6 bg-gray-50 border border-gray-200"):
            ui.label("Фактическое платежное поведение портфеля").classes("font-bold text-base")
            ui.label(
            ...
            with ui.column().classes("gap-2"):
                with ui.row().classes("items-center gap-3"):
                    ...
                    with ui.icon("info_outline").classes("text-gray-500 cursor-help"):
                        ui.tooltip(
                            "Профиль рассчитан по последним оплаченным накладным портфеля. "
                            "Накладные с фактическим сроком оплаты 0 дней учитываются только для ARS_NEW. "
