...
SELECT
    ...
    CASE
        WHEN SUM(CASE WHEN is_overdue_real THEN invoice_amount ELSE 0 END) > 0 THEN 'HIGH'
        WHEN SUM(CASE WHEN is_due_today THEN invoice_amount ELSE 0 END) > 0 THEN 'HIGH'
        WHEN SUM(CASE WHEN is_due_in_3_days THEN invoice_amount ELSE 0 END) > 0 THEN 'MEDIUM'
        WHEN SUM(CASE WHEN is_due_in_7_days THEN invoice_amount ELSE 0 END) > 0 THEN 'MEDIUM'
        ELSE 'LOW'
    END AS risk_category,
    CASE
        WHEN SUM(CASE WHEN is_overdue_real THEN invoice_amount ELSE 0 END) > 0 THEN 'CALL NOW'
        WHEN SUM(CASE WHEN is_due_today THEN invoice_amount ELSE 0 END) > 0 THEN 'CONTROL TODAY'
        WHEN SUM(CASE WHEN is_due_in_3_days THEN invoice_amount ELSE 0 END) > 0 THEN 'REMIND'
        WHEN SUM(CASE WHEN is_due_in_7_days THEN invoice_amount ELSE 0 END) > 0 THEN 'MONITOR'
        ELSE 'OK'
