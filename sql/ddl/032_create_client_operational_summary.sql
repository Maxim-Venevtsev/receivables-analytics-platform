-- ============================================================
-- 032_create_client_operational_summary.sql
-- Purpose:
--   Client-level operational summary for reusable client tables.
--   Adds inferred contract payment term and operational risk fields.
-- ============================================================

DROP VIEW IF EXISTS core.v_client_operational_summary CASCADE;
...
client_agg AS (
    SELECT
        ...
        COUNT(*) FILTER (WHERE COALESCE(term_shift_count, 0) > 0) AS shifted_invoice_count,
        COUNT(*) FILTER (WHERE COALESCE(term_shift_count, 0) >= 2) AS repeated_shift_invoice_count,
        ...
        SUM(CASE WHEN payment_term_days >= 45 THEN invoice_amount ELSE 0 END) AS debt_45_plus,
        SUM(CASE WHEN payment_term_days >= 60 THEN invoice_amount ELSE 0 END) AS debt_60_plus,
        SUM(CASE WHEN payment_term_days >= 90 THEN invoice_amount ELSE 0 END) AS debt_90_plus,
        SUM(CASE WHEN payment_term_days >= 120 THEN invoice_amount ELSE 0 END) AS debt_120_plus,

        SUM(
            CASE
                WHEN NOT is_overdue_real
                 AND payment_term_days >= 45
                THEN invoice_amount
                ELSE 0
            END
